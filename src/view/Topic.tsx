...
import { getAllQuestions, countQuestionsByTopic } from "../service/api";
import { Question, GroupedDataTopic } from "../types/type";
...
function Topic() {
  ...
  const [groupedData, setGroupedData] = useState<GroupedDataTopic[]>([]);
  ...
  const navigation = groupedData.map((item) => ({
    name: item.topic,
    count: item.count,
    current: currentTab === item.topic,
  }));

  const filteredQuestions = questions.filter(
    (question) => question.topic === currentTab
  );
  ...
  useEffect(() => {
    async function initialSetUp() {
      ...
      try {
        ...
        setQuestions(data);
        const temp = await countQuestionsByTopic();
        setGroupedData(temp);
      } catch (error) {
        console.error(error);
      } finally {
